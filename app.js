...
'use strict';
const googleTranslate = require('google-translate')('AIzaSyDYsStRWHbyPg0XPpD9Y5eRPubV1L0IB5k');
...
app.post('/api/message', function(req, res) {
  ...
  bankingServices.getPerson(7829706, function(err, person) {
    ...
    if (req.body) {
      if (req.body.input) {
        var inputstring_original = req.body.input.text;
        let inputstring = '';
    ...
    callAssistant(payload, inputstring_original);
  });
      ...
      if (payload.input.text != '') {
        ...
        nlu.analyze(parameters, function(err, response) {
          ...
          assistant.message(payload, async function(err, data) {
            console.log(inputstring_original);

            googleTranslate.detectLanguage(inputstring_original, async function(err, detection) {
              let inputLanguage = '';
              if (err) throw err;
              ...
              if (!isNaN(parseInt(inputstring_original))) {
                ...
                await res.cookie('userData', users);
              }
              ...
              if (data.output.text.length == 0) {
                data.output.text[0] = 0;
              }
              // const tempstr = data.output.text[0].indexOf('<');
              // console.log(tempstr);
              ...
              if (err) {
                ...
                checkForLookupRequests(data, function(err, data) {
                  if (err) {
                    ...
                    googleTranslate.translate(data.output.text[0], inputLanguage, function(err, translation) {
                      // if (err) throw err;
                      data.output.text[0] = translation.translatedText;
                      console.log('assistant.message :: ', JSON.stringify(data));
        ...
        assistant.message(payload, function(err, data) {
          let inputlanguage = '';
          googleTranslate.detectLanguage(inputstring_original, function(err, detection) {
            if (err) throw err;
            else inputlanguage = detection.language;

            googleTranslate.translate(data.output.text[0], inputlanguage, function(err, translation) {
              if (err) throw err;
              else data.output.text[0] = translation.translatedText;
              if (err) {
                return res.status(err.code || 500).json(err);
              } else {
                // =>  es
